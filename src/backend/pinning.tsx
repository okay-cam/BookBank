...
export async function togglePinListing(listing: Listing) {
  ...
  const q = query(pinsRef, where(listings_field.userID, "==", userId), where(listings_field.listingID, "==", listing.id));
...
export const isPinned = async (listingId: string): Promise<boolean> => {
  ...
  const q = query(pinsRef, where(listings_field.userID, "==", userId), where(listings_field.listingID, "==", listingId));
